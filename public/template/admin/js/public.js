...
$(function () {
    ...
    $("#configuration").change(function (e) {
        e.preventDefault();
        let configuration_id = $(this).val();

        $.ajax({
            type: "POST",
            url: "/admin/ajax/getConfiguration",
            data: { configuration_id },
            dataType: "JSON",
            success: function (res) {
                if (res.isSuccess) {
                    updateRowIntoList(res.configuration, "configuration");
                } else {
                    alert("Vui lòng chọn lại cấu hình !");
                }
            },
        });
    });
    ...
    $("select[name=parentCategory]").change(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/ajax/getFirstSubCategories",
            data: { parentCategory_id },
            dataType: "JSON",
            success: function (res) {
                if (res.isSuccess) {
                    updateSelectBox(
                        res.firstSubCategories,
                        "select[name=firstSubCategory]"
                    );
                }
            },
        });
    ...
    $("select[name=firstSubCategory]").change(function (e) {
        e.preventDefault();
        let firstSubCategory_id = $(this).val();

        $.ajax({
            type: "POST",
            url: "/admin/ajax/getSecondSubCategories",
            data: { firstSubCategory_id },
            dataType: "JSON",
            success: function (res) {
                if (res.isSuccess) {
                    updateSelectBox(
                        res.secondSubCategories,
                        "select[name=secondSubCategory]"
                    );
                }
            },
        });
    ...
    //Thay đổi province
    $("#delivery_province").change(function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            //   alert(data.districts);
            updateSelectBox(data.districts, "#delivery_district");
        });

        $("#delivery_district").change(function (e) {
            e.preventDefault();
            var district_id = $(this).val();
            ...
            $.ajax({
                ...
                //   alert(data.districts);
                updateSelectBox(data.wards, "#delivery_ward");
            });
        });
    });
...
function updateRowIntoPromotionList(data, qty) {
    ...
    const prevRowEle = $(".tbody-productlist").html();
...
function addPromotionInPackage(promotion_id, type) {
    let qty = $(".promotion_quantity").val();
...
function minusQuantity(tag) {
    const qty = $(tag).next().val();

    $("input[name=qty_in_cart]").val(qty);
}

function plusQuantity(tag) {
    const qty = $(tag).prev().val();
...
function updateRowIntoList(data, type) {
    if (type == "origin") {
        ...
        rows += `<tr class="info-order-items">
            <input type="hidden" name="origin_ids[]" value="${data.id}">
            <td>${data.id}</td>
            <td>${data.name}</td>
        ...
        if (!data) {
            $("#configuration_table .tbody-list").html(`<tr class="no-data">
            <td>No data</td>
            <td>No data</td>
            <td>No data</td>
        ...
        // $(".tbody-list .info-order-items").remove();
        $("#configuration_table .tbody-list .no-data").remove();

        var rows = "";

        rows += `<tr class="info-order-items">
            <input type="hidden" name="configuration_ids[]" value="${data.id}">
            <td>${data.id}</td>
            <td>${data.name}</td>
            <td><a class="btn btn-danger remove-configuration" href="javascript:void(0)" onclick="deleteItemInBoughtList(this)"><i class="fa fa-trash" aria-hidden="true"></i></a>
            </td>
           </tr>`;

        $("#configuration_table .tbody-list").append(rows);
        ...
        rows += `<tr class="info-order-items">
            <input type="hidden" name="feature_ids[]" value="${data.id}">
            <td>${data.id}</td>
            <td>${data.name}</td>
